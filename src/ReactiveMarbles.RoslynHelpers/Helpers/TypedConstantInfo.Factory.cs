...
/// <inheritdoc/>
public partial record TypedConstantInfo
{
    /// <summary>
        ...
        [NotNullWhen(true)] out TypedConstantInfo? info)
    {
        ...
        if (operation is IArrayCreationOperation)
        {
            ...
            foreach (var initializationExpression in initializerExpression.Expressions)
            {
                ...
                if (!TryCreate(initializationOperation, semanticModel, initializationExpression, token, out var elementInfo))
                {
                    goto Failure;
                }
